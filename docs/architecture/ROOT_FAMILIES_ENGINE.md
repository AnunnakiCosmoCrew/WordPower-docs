...
>
> **Spike D outcome (2026-05-11):** Comparative survey of Gemini 2.5 Flash vs Haiku 4.5 on the same 81-word test set with extended prompt. **Gemini 2.5 Flash wins:** accuracy 97.9% (vs Haiku's 93.75% on the same extended prompt), 10/10 trap refusal, top-30k cost **$61** (vs Haiku $87, -29%). Decision rule (exceeds quality at ≤ same cost) triggered. **L1 primary switches from Claude Haiku 4.5 to Gemini 2.5 Flash.** Haiku stays in the architecture as fallback. See [Spike D FINDINGS](../../spikes/morphology-engine/d-model-survey/FINDINGS.md) and [`ROOT_FAMILIES_DECISION.md`](ROOT_FAMILIES_DECISION.md) for the locked architecture.
