...
>
> **Spike D outcome (2026-05-11 morning):** Comparative survey of Gemini 2.5 Flash vs Haiku 4.5 on the same 81-word test set with prompt-v1 (locked under #525). Gemini measured at accuracy 97.9% (47/48, vs Haiku's 95.8% / 46/48), 10/10 trap refusal, top-30k cost $61 (vs Haiku $87, projected -30%). Decision rule (exceeds quality at ≤ same cost) initially triggered → architecture briefly switched to Gemini.
>
> **Top-1k production validation outcome (2026-05-11 evening):** Gemini 2.5 Flash run on the same 1000 SUBTLEX-US production words used by PR #539's Haiku pilot. **The Spike D cost projection did not hold at production scale.** Headline: Gemini agreement with Haiku 90.1% (equivalent quality); Gemini cost $1.90 vs Haiku $1.71 for 1000 words (**+11% more expensive, not -30% cheaper**); Gemini wall-clock 42 min vs Haiku 7.5 min. Cause: Anthropic's explicit `cache_control` hits ~100% of calls after the first; Gemini's implicit caching is sporadic. The 51-word Spike D test set was too small to reveal this caching dynamic. **Per the same locked decision rule that originally triggered the Gemini switch: confirm Haiku.** L1 primary reverts to Claude Haiku 4.5 with documented evidence; Gemini stays in the architecture as a secondary provider (drop-in replacement if Anthropic has quota/availability issues). See [top-1k comparison](../../pipeline/validation/top1k-comparison.md) and [`ROOT_FAMILIES_DECISION.md`](ROOT_FAMILIES_DECISION.md) for the final locked architecture.
